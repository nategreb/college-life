"""Tests for the models of the reviews app."""
...
class ReviewTestCase(TestCase):
    ...
    def setUp(self):
        self.review = mixer.blend(
            'reviews.Review', content_type=ContentType.objects.get_for_model(
                WeatherCondition))
    ...
    def test_get_average_rating(self):
        ...
        mixer.blend('reviews.Rating', review=self.review, value='2')
        mixer.blend('reviews.Rating', review=self.review, value='4')
        self.assertEqual(self.review.get_average_rating(), 3, msg=(
            'Should return the average rating value.'))

        mixer.blend('reviews.Rating', review=self.review, value='')
        mixer.blend('reviews.Rating', category__counts_for_average=False,
    ...
    def test_get_average_rating_with_custom_choices(self):
        self.assertFalse(self.review.get_average_rating(), msg=(
            'If there are no ratings, it should return False.'))
        rating1 = mixer.blend('reviews.Rating', review=self.review, value='4')
        # we create choices to simulate, that the previous value was the max
        for i in range(0, 5):
            mixer.blend('reviews.RatingCategoryChoiceTranslation',
                        language_code='en-us',
                        ratingcategory=rating1.category, value=i)
        rating2 = mixer.blend('reviews.Rating', review=self.review, value='6')
        # we create choices to simulate, that the previous value was the max
        for i in range(0, 7):
            mixer.blend('reviews.RatingCategoryChoiceTranslation',
                        language_code='en-us',
                        ratingcategory=rating2.category, value=i)
        mixer.blend('reviews.Rating', category=rating2.category,
        ...
        self.assertEqual(
            self.reviews.get_category_averages(6),
            {rating1.category: 6.0, rating2.category: 6.0},
            msg=('Should return the average ratings for the category.'))

        self.assertEqual(
            self.reviews.get_category_averages(),
            {rating1.category: 6.0, rating2.category: 6.0},
            msg=('Should return the average ratings for the category.'))

        self.assertEqual(
            self.reviews.get_category_averages(100),
        ...
        mixer.blend('reviews.Rating', reviews=self.reviews, value='')
        self.assertEqual(self.reviews.get_average_rating(6), 6, msg=(
            'Should return the average rating value.'))
        self.assertEqual(self.reviews.get_average_rating(4), 4, msg=(
            'Should return the average rating value.'))
        self.assertEqual(self.reviews.get_average_rating(100), 100, msg=(
            'Should return the average rating value.'))
        ...
        rating3.value = '1'
        rating3.save()
        self.assertEqual(self.reviews.get_average_rating(6), 1.25,
                         msg=('Should return the average rating value.'))
        self.assertEqual(self.reviews.get_average_rating(4), 0.8333333333333333,
                         msg=('Should return the average rating value.'))
        self.assertEqual(
            self.reviews.get_average_rating(100),
        ...
        rating3.value = '0'
        rating3.save()
        self.assertEqual(self.reviews.get_average_rating(6), 0,
                         msg=('Should return the average rating value.'))
        self.assertEqual(self.reviews.get_average_rating(4), 0,
                         msg=('Should return the average rating value.'))
        self.assertEqual(self.reviews.get_average_rating(100), 0,
                         msg=('Should return the average rating value.'))
...
class RatingCategoryTestCase(TestCase):
    ...
    def setUp(self):
        self.category = mixer.blend('reviews.RatingCategoryTranslation',
                                    language_code='en-us')
...
class RatingCategoryChoiceTestCase(TestCase):
    ...
    def test_instantiation(self):
        ...
        ratingcategorychoice = mixer.blend(
            'reviews.RatingCategoryChoiceTranslation', language_code='en-us')
...
class RatingTestCase(TestCase):
    ...
    def setUp(self):
        review = mixer.blend(
            'reviews.Review', content_type=ContentType.objects.get_for_model(
                WeatherCondition))
        self.rating = mixer.blend('reviews.Rating', review=review)
