"""Test for the template tags of the ``reviews`` app."""
...
class GetReviewAverageTestCase(TestCase):
    ...
    def test_tag(self):
        ...
        review = mixer.blend(
            'reviews.Review', reviewed_item=self.reviewed_item,
            content_type=ContentType.objects.get_for_model(WeatherCondition),
            object_id=self.reviewed_item.pk,
        )
        self.assertEqual(review_tags.get_review_average(self.reviewed_item), 0)
        rating = mixer.blend('reviews.Rating', review=review)
        mixer.blend('reviews.Rating', review=review, category=rating.category)
        """
        Fix those tests!
...
class RenderCategoryAveragesTestCase(TestCase):
    ...
    def test_tag(self):
        ...
        self.rating = mixer.blend(
            'reviews.Rating', review=self.review, value='2')
        expected_value = {
            'reviewed_item': self.reviewed_item,
    ...
    def test_tag_more_extensively(self):
        ...
        rating2 = mixer.blend('reviews.Rating', review=self.review, value='4')
        # we create choices to simulate, that the previous value was the max
        for i in range(0, 7):
            mixer.blend('reviews.RatingCategoryChoiceTranslation',
                        language_code='en-us',
...
class TotalReviewAverageTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.review = mixer.blend(
            'reviews.Review', reviewed_item=self.content_object,
            content_type=ContentType.objects.get_for_model(WeatherCondition),
        )
        self.rating1 = mixer.blend('reviews.Rating', review=self.review,
                                   value='4')
        # we create choices to simulate, that the previous value was the max
        for i in range(0, 5):
            mixer.blend('reviews.RatingCategoryChoiceTranslation',
        ...
        self.rating2 = mixer.blend('reviews.Rating', review=self.review,
                                   value='6')
        # we create choices to simulate, that the previous value was the max
        for i in range(0, 7):
            mixer.blend('reviews.RatingCategoryChoiceTranslation',
    ...
    def test_tag(self):
        ...
        mixer.blend('reviews.Rating', category=self.rating1.category,
                    review=self.review, value='0')
        mixer.blend('reviews.Rating', category=self.rating2.category,
                    review=self.review, value='0')
        self.assertEqual(
...
class UserHasReviewedTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.content_object = mixer.blend('test_app.WeatherCondition')
        self.review = mixer.blend(
            'reviews.Review', user=self.user, reviewed_item=self.content_object,
            content_type=ContentType.objects.get_for_model(WeatherCondition),
        )
